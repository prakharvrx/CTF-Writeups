...
We are given `dis.txt` which is as follows:
...
The above disassembly can be reversed to find the flag using the script `exploit.py` which is as follows:

```python
#!/usr/bin/python3

def cipherOperation(c1, c2):
	return ord(c1)^c2

output = 'éÿîÅËÎÞÃÙóÙÕÎÈÊúèÞÎÜÌÌÕÓÕìùÂéçÆÐþÿñÖËîÿôÿ'
decodedDecArray = []
for i in range(len(output)):
	a = cipherOperation(output[i], i)
	a ^= 170
	decodedDecArray.append(a)

print("Flag: " + ''.join(map(chr, decodedDecArray)))
```
...
Finally the flag becomes: `CTFlearn{Python_Reversing_Is_Pretty_Easy}`
